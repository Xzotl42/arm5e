...
export class ArM5eNPCActorSheetV2 extends Arm5eCharacterActorSheetV2 {
  ...
  _configureRenderOptions(options) {
    ...
    if (this.document.limited) {
      options.parts = Object.keys(ArM5eNPCActorSheetV2.LIMITED_PARTS);
  ...
  _configureRenderParts(options) {
    ...
    if (this.actor.system.charType?.value !== "magusNPC") {
      delete parts.arts;
  ...
  async _prepareContext(options) {
    ...
    await this._prepareCharacterContext(context, { showQualitiesForEntity: true });
  ...
  isItemDropAllowed(itemData) {
    switch (itemData?.type) {
      ...
      case "power":
        return this.actor.system.charType.value === "entity";
      case "spell":
      case "magicalEffect":
