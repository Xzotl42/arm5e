...
export class LabActivity extends Activity {
  ...
  static LabActivityFactory(labUuid, ownerUuid, type) {
    switch (type) {
      case "none":
        return new NoLabActivity(labUuid);
      case "inventSpell":
      case "learnSpell":
        return new SpellActivity(labUuid, ownerUuid, type);
      case "minorEnchantment":
        return new MinorEnchantment(labUuid, ownerUuid);
      case "chargedItem":
        return new ChargedItem(labUuid, ownerUuid);
      case "visExtraction":
        return new VisExtractionActivity(labUuid, ownerUuid);
      case "longevityRitual":
        return new LongevityRitualActivity(labUuid, ownerUuid);
      case "investigateItem":
        return new InvestigationActivity(labUuid, ownerUuid);
      default:
        log(false, "Unknown activity");
        return new SpellActivity(labUuid, ownerUuid, "inventSpell");
    }
  }
  ...
  computeLabTotal(lab, actor, data, options = { distractions: "none", focus: false }) {
    return this._computeLabTotal(lab, actor, data, options);
  }

  _computeLabTotal(lab, actor, data, options = { distractions: "none", focus: false }) {
    let labTot = computeRawCastingTotal(data, actor, options);
...
export class SpellActivity extends LabActivity {
  ...
  getTargetLevel(planning) {
    return computeLevel(planning.data.system, "spell");
  }
